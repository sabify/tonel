        ...
        match self.state {
            State::Established => {
                self.incoming.recv_async().await.ok().and_then(|raw_buf| {
                    ...
                    let last_ask = self.last_ack.load(Ordering::SeqCst);
                    ...
                    match time::timeout(TIMEOUT, self.incoming.recv_async()).await {
                        Ok(buf) => {
                                    ...
                                    .is_ok()
                            {
                                // found our SYN + ACK
                                self.ack
                                    .store(tcp_packet.get_sequence() + 1, Ordering::SeqCst);
    ...
    pub async fn connect(&self, addr: SocketAddr) -> Option<Socket> {
        for _ in 1024..u16::MAX {
            ...
            let mut sock = match self.shared.tuples.entry(tuple) {
                ...
                Entry::Vacant(v) => {
                    let tun_index = self.shared.tun_index.fetch_add(1, Ordering::SeqCst)
                        % self.shared.tun.len();
                        ...
                        Some((ip_packet, tcp_packet)) => {
                                    ...
                                    .contains(&tcp_packet.get_destination())
                            {
                                ...
                                if tcp_packet.get_sequence() == 0 {
                                    ...
                                    let tun_index = shared.tun_index.fetch_add(1, Ordering::SeqCst) % shared.tun.len();
